...
class hybridcd(ConflictDetection):
    ...
    def detect(self, ownship, intruder, rpz, hpz, dtlookahead):
        ...
        if traf.swintent:
            confpairs, inconf, swconfl = self.intentFilter(traf.cd, confpairs, inconf, ownship, intruder, swverconf, swconfl)

        filterconfs = np.reshape(inconf,(len(inconf),1))*inconf*swconfl
        ########## Finaly return with the filtered confpairs and inconf! ##########

        return confpairs, lospairs, inconf, tcpamax, \
            qdr[filterconfs], dist[filterconfs], np.sqrt(dcpa2[filterconfs]), \
                tcpa[filterconfs], tinconf[filterconfs]
